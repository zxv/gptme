...
parser = argparse.ArgumentParser(
    description="Contextual generation (aka given some metadata we will generate articles"
)
parser.add_argument(
    "-metadata_fn",
    dest="metadata_fn",
...
parser.add_argument("-input", dest="input", type=str, help="Text to complete")
parser.add_argument(
    "-model_config_fn",
...
parser.add_argument("-samples", dest="samples", default=1, type=int, help="num_samples")
...
~~\
".format(
        args.batch_size, max_batch_size, num_chunks, batch_size_per_chunk
    ),
    flush=True,
)
        ...
        for chunk_i in range(num_chunks):
            ...
            for t_i, p_i in zip(tokens_out, probs_out):
                extraction = extract_generated_target(
                    output_tokens=t_i, encoder=encoder, target=args.target
                )
